...
- Create: `apps/booth/test/booth_session_test.dart`, `apps/booth/test/operator_gesture_test.dart`, `apps/booth/test/timed_screens_test.dart`
...
2. `_buildStage()`의 `report` 분기에 `onHistory: () => setState(() => _stage = BoothStage.history)`를 넣는다
