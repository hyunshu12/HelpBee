...
from app.services.crops import crop_pad, normalize_box  # noqa: F401 - normalize_box 재수출
...
def crop_pad_224(img, box, margin=0.10, size=224):
    ...
    규칙 본체는 app/services/crops.crop_pad (서빙과 단일 소스)."""
    return crop_pad(img, box, margin=margin, size=size)
