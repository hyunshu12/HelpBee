...
외부(Task 4): VarroaDataset 은 이미 벌 1마리 크롭(그 boxes 는 응애 위치) → 이미지 전체를 크롭 박스로.
EV2 는 boxes[0] 이 벌 박스(프레임 좌표). 두 소스 모두 같은 crop_pad_224. split 은 VarroaDataset 자체
split, EV2 는 split_external(영상 단위 holdout).
...
def external_crop_box(row: dict, shape) -> tuple:
    if row["source"] == "varroadataset":  # 이미 벌 크롭 — boxes 는 응애 위치
        return (0, 0, shape[1], shape[0])
    return tuple(row["boxes"][0])  # EV2: 벌 박스
...
def crops_71667(manifest: dict, weights: dict[str, Path], out: Path, writer) -> dict:
    ...
    for image, meta in sorted(manifest["images"].items()):
        ...
        for i, (box, cat) in enumerate(m):
            ...
            writer.writerow([rel, label, meta.get("source", "71667"), meta.get("colony", ""), meta.get("device", ""),
                             meta["split"], nw, nh, cat, image, ""])
            n_crops += 1
    return {"matched": matched, "total": total, "zero_match_images": zero_match_images, "crops_71667": n_crops}


def crops_external(rows: list[dict], root: Path, splits: dict[str, str], out: Path, writer) -> int:
    n = 0
    for r in rows:
        ...
        writer.writerow([rel, r["label"], r["source"], "", "", split, nw, nh, "", Path(r["image"]).as_posix(), vis])
        n += 1
    return n
...
def main() -> None:
    ...
    with (a.out / "crops.csv").open("w", encoding="utf-8", newline="") as f:
        ...
        for rows, root in ext_rows:
            stats[f"crops_{rows[0]['source'] if rows else 'external'}"] = crops_external(rows, root, splits, a.out, w)
